...
8. **Offer agent-guided setup with explicit user consent.** If a candidate repository is detected (state `candidate_repo`), do not run `/superctx:init` or mutate files without the user's explicit natural-language consent (e.g., asking 'Would you like me to set up SuperCtx to manage your project context across agents?'). Once consent is given, run `/superctx:init`.
9. **Guide legacy or broken setups to repair/migration.**
   - If the state is `managed_needs_repair`, run `/superctx:status` and `/superctx:sync` to restore shims from the hub.
